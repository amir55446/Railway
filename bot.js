...
// ── مسح كل ملفات مجلد الأغاني (مع تأكيد) ──────────────────
function cleanAudioDir() {
  if (!fs.existsSync(CONFIG.AUDIO_DIR)) return;
  const files = fs.readdirSync(CONFIG.AUDIO_DIR);
  let deleted = 0;
  for (const f of files) {
    const filePath = path.join(CONFIG.AUDIO_DIR, f);
    try {
      fs.unlinkSync(filePath);
      deleted++;
      console.log(`🗑️ تم مسح: ${f}`);
    } catch (e) {
      console.error(`❌ فشل مسح ${f}:`, e.message);
    }
  }
  if (deleted > 0) console.log(`✅ تم مسح ${deleted} ملف من temp_audio`);
}
...
function downloadYouTubeAudio(songName) {
  return new Promise((resolve, reject) => {
    ...
    // امسح أي ملفات قديمة قبل التحميل
    cleanAudioDir();
    ...
    exec(command, { timeout: 180000, maxBuffer: 10 * 1024 * 1024 }, (error, stdout, stderr) => {
      if (error) {
        ...
        console.log('🔄 محاولة تانية بدون صورة مصغرة...');
        const cmd2 = buildYtDlpCommand(YTDLP, safeName, outputTemplate)
          .replace('--write-thumbnail --convert-thumbnails jpg ', '');
        exec(cmd2, { timeout: 180000, maxBuffer: 10 * 1024 * 1024 }, (err2, out2, serr2) => {
          if (err2) {
            console.error('❌ yt-dlp retry stderr:', serr2?.slice(0, 300));
            cleanAudioDir(); // مسح أي ملفات ناقصة
          ...
          if (!result.audioPath) {
            cleanAudioDir();
            reject(new Error('لم يتم العثور على ملف MP3'));
            return;
          }
          resolve(result);
        });
      ...
      if (!result.audioPath) {
        cleanAudioDir();
        reject(new Error('لم يتم العثور على ملف MP3 بعد التحميل'));
        return;
      }
      resolve(result);
    });
...
client.on('group_join', async (notification) => {
  ...
  try {
    ...
    // فحص تعطيل الترحيب في هذا الجروب تحديداً
    if (botSettings.welcomeEnabledGroups[chat.id._serialized] === false) return;

    // ✅ جلب اسم العضو الجديد (الاسم فقط وليس الرقم)
    let memberName = null;
    try {
    ...
    // لو مش لاقي اسم → اكتب "عضو جديد" بدل الرقم
...
async function handleGroupMessage(msg, chat) {
  ...
  if (youtubeMatch && botSettings.youtubeEnabled) {
    ...
    try {
      ...
      // إرسال الصورة المصغرة أولاً لو موجودة
      ...
      console.log(`✅ تم إرسال أغنية: ${songName}`);
    } catch (err) {
      try { await waitMsg.delete(true); } catch (_) {}
      await msg.reply(`❌ فشل تحميل الأغنية\n${err.message}`);
      console.error('❌ YouTube error:', err.message);
    } finally {
      // ✅ مسح كل الملفات بعد الإرسال أو الفشل — مضمون دايماً
      cleanAudioDir();
    }
    return;
  }
