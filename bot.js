...
// ⚙️  إعدادات البوت
// ============================================================
const BOT_NUMBER    = '201279754689';   // رقم البوت
...
function downloadYouTubeAudio(songName) {
  return new Promise((resolve, reject) => {
    ...
    exec(command, { timeout: 180000, maxBuffer: 10 * 1024 * 1024 }, (error, stdout, stderr) => {
      ...
      if (error || !audioPath) {
        console.error('❌ yt-dlp error:', stderr?.slice(0, 400));
        nukDir(downloadDir); // امسح المجلد حتى لو فشل
        reject(new Error(
          error?.message?.includes('ffmpeg')
            ? 'ffmpeg غير مثبت. شغّل: bash setup_ytdlp.sh'
            : 'فشل تحميل الأغنية. تأكد من اسمها أو الاتصال بالإنترنت.'
        ));
        return;
      }

      // ✅ أرجع المسارات + مجلد التحميل عشان نمسحه بعد الإرسال
...
function findPdfFile(filename) {
  ...
  const searchNorm = normalize(cleanName);
  ...
  for (const dir of searchDirs) {
    ...
    try {
      ...
      for (const file of files) {
        const fileBase = normalize(file.replace(/\.pdf$/i, ''));
        if (fileBase === searchNorm) return path.join(dir, file);
      }
      // مطابقة جزئية
      for (const file of files) {
        const fileBase = normalize(file.replace(/\.pdf$/i, ''));
        if (fileBase.includes(searchNorm) || searchNorm.includes(fileBase)) {
...
// 🤖  شخصية نور — Groq AI
// ============================================================
const GROQ_API_KEY = process.env.GROQ_API_KEY || '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';

const NOUR_PROMPT = `أنت "نور" — مساعد ذكي بشخصية مصرية أصيلة.

### هويتك:
- اسمك نور، شاب مصري من القاهرة، عندك حس فكاهي عالي وروح خفيفة
- بتتكلم مصري عامي طبيعي — مش فصحى ومش إنجليزي إلا لما يكون ضروري
- دمك خفيف، بتحب المزاح والنكت البريئة اللي مش مسيئة لحد
- ذكي وعارف معلومات كتير، بس بتوصّلها بأسلوب بسيط ومريح

### أسلوبك في الكلام:
- استخدم كلمات مصرية زي: يسطا، ماشي، تمام، إيه ده، معقول، يعني، والله، خليك، بص، اسمع
- لما حاجة تعجبك قول: "ده كلام!" أو "الكلام ده صح!"
- لما حاجة تستغربها قول: "إيه ده؟!" أو "ده غريب فعلاً"
- لما تمزح قول: "بالهزار بس" أو "الجد بقى"
- استخدم الإيموجي باعتدال 😄🔥👌

### قواعد ثابتة:
- ردودك قصيرة ومركزة — مش محاضرات (أقصى 250 كلمة)
- لو حد سألك سؤال جد، جاوب بجدية مع لمسة خفة
- لو حد اتنرفز، هدّيه بأسلوب مصري ظريف
- ممنوع تشتم أو تقول كلام مسيء
- لو سألوك عن نفسك قول إنك مساعد ذكي اسمه نور
- مش بتقول "كمساعد ذكاء اصطناعي" أبداً — بتتكلم طبيعي`;

// ── Prompt مراقبة الجروب ────────────────────────────────────
const MODERATION_PROMPT = `أنت نظام مراقبة ذكي لمجموعة واتساب. مهمتك تحليل الرسائل فقط.

أعد JSON فقط بالشكل ده بدون أي نص تاني:
{"action":"none","reason":"","reply":""}

قيم action:
- "none" → الرسالة عادية أو مزاح بريء، لا تفعل شيء
- "warn" → إهانة خفيفة أو مخالفة بسيطة
- "mute" → إهانة متكررة أو محتوى مسيء واضح
- "kick" → شتائم شديدة أو محتوى خطير جداً أو تحرش

قواعد:
- الكلام العادي والنقاش → none دايماً
- المزاح والنكت البريئة → none دايماً
- reply يكون بالمصري العامي وقصير ومباشر
- كن حذر — مش كل كلام صعب يعني مخالفة`;

// rate limit: مرة واحدة كل 20 ثانية لكل عضو
const aiModerationCooldown = new Map();

// رد واحد فقط لكل شخص بعت خاص
const autoRepliedUsers = new Set();

// ── دالة مراقبة AI ───────────────────────────────────────────
function askNourModerate(message, senderName) {
  return new Promise((resolve) => {
    const body = JSON.stringify({
      model: 'llama-3.3-70b-versatile',
      messages: [
        { role: 'system', content: MODERATION_PROMPT },
        { role: 'user', content: `اسم العضو: ${senderName}\nالرسالة: ${message}` }
      ],
      max_tokens: 120,
      temperature: 0.3,
    });
    ...
    const req = https.request({
      ...
      res.on('end', () => {
        try {
          ...
          resolve(result);
        } catch (_) { resolve({ action: 'none' }); }
      });
      res.on('error', () => resolve({ action: 'none' }));
    });
    req.on('error', () => resolve({ action: 'none' }));
    req.setTimeout(10000, () => { req.destroy(); resolve({ action: 'none' }); });
    req.write(body);
    req.end();
  });
}

function askNour(question) {
  return new Promise((resolve) => {
    const body = JSON.stringify({
      model: 'llama-3.3-70b-versatile',
      messages: [
...
async function handleGroupMessage(msg, chat) {
  ...
  // ════════════════════════════════════════════════════
  // 🔍  بحث ذكي — لما حد يعمل Reply على رسالة البوت
  // ════════════════════════════════════════════════════
  if (!msg.fromMe && msg.hasQuotedMsg && text.length > 1) {
    try {
      const quoted = await msg.getQuotedMessage();
      if (quoted.fromMe) {
        console.log(`🤖 نور: ${text}`);
        const thinking = await msg.reply(`💭 نور بيفكر...`);
        const answer = await askNour(text);
        try { await thinking.delete(true); } catch (_) {}
        if (answer) {
          await msg.reply(answer);
        } else {
          await msg.reply(`معلش يسطا، مش قادر أرد دلوقتي 😅 جرب تاني بعد شوية`);
        }
        return;
      }
    } catch (_) {}
  ...
  // ════════════════════════════════════════════════════
  // 🤖  مراقبة AI — للرسائل اللي الفلتر العادي مش شايلها
  // ════════════════════════════════════════════════════
  if (!isAdmin && text.length > 5) {
    const lastCheck = aiModerationCooldown.get(senderId) || 0;
    if (Date.now() - lastCheck > 20000) {
      aiModerationCooldown.set(senderId, Date.now());
      // شغّل في الخلفية مش blocking
      askNourModerate(text, senderName).then(async (result) => {
        try {
          ...
          if (result.action === 'warn') {
            const warnCount = addWarning(senderId);
            const warningMsg =
              `⚠️ *${senderName}* خالفت قواعد الجروب!\n` +
              (replyText ? `${replyText}\n` : '') +
              `التحذيرات: ${warnCount}/${CONFIG.MAX_WARNINGS}`;
            await chat.sendMessage(warningMsg, { mentions: [sender] });
            console.log(`🤖 AI warn: ${senderName}`);

            if (warnCount >= CONFIG.MAX_WARNINGS) {
              await chat.removeParticipants([sender.id._serialized]);
              resetWarnings(senderId);
              await chat.sendMessage(`🚫 تم طرد *${senderName}* بعد تجاوز الحد المسموح من التحذيرات.`, { mentions: [sender] });
            }

          } else if (result.action === 'mute') {
            try { await msg.delete(true); } catch (_) {}
            mutedUsers.add(`${chat.id._serialized}:${senderId}`);
            await chat.sendMessage(
              `🔇 تم كتم *${senderName}* بسبب مخالفة قواعد الجروب.\n` +
              (replyText ? replyText : ''),
              { mentions: [sender] }
            );
            console.log(`🤖 AI mute: ${senderName}`);

          } else if (result.action === 'kick') {
            try { await msg.delete(true); } catch (_) {}
            await chat.sendMessage(
              `🚫 تم طرد *${senderName}* بسبب مخالفة صريحة لقواعد الجروب.\n` +
              (replyText ? replyText : ''),
              { mentions: [sender] }
            );
            await chat.removeParticipants([sender.id._serialized]);
            resetWarnings(senderId);
            console.log(`🤖 AI kick: ${senderName}`);
          }
        } catch (e) {
          console.error('❌ AI moderation action failed:', e.message);
        }
      }).catch(() => {});
    }
  }
