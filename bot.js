...
async function handleSelfChat(msg, chat, text) {
  ...
  if (selfState.phase === 'waiting_report_number') {
    ...
    if (!rawNum) { await msg.reply('❌ اكتب الرقم صح (أرقام فقط).'); return; }
    let number = rawNum;
    if (number.startsWith('01') && number.length === 11) number = '2' + number;
    else if (number.startsWith('0')) number = '2' + number.slice(1);
    selfState.phase = `waiting_report_count:${number}`;
    await msg.reply(
      `📋 الرقم: *+${number}*\n\n` +
      `❓ كم عدد البلاغات اللي عايزها؟\n(اكتب رقم من 1 لـ 20)`
    );
    return;
  }
  ...
  if (selfState.phase.startsWith('waiting_report_count:')) {
    ...
    if (isNaN(count) || count < 1 || count > 20) {
      await msg.reply('❌ اكتب رقم من 1 لـ 20 بس.');
      return;
    }
    selfState.phase = 'idle';

    // ابدأ الإبلاغ
    const progressMsg = await msg.reply(`⏳ *جاري تحميل البلاغات...*\n\n0 / ${count} تم تنفيذه`);
    ...
    for (let i = 0; i < count; i++) {
      try {
        ...
        await client.pupPage.evaluate(async (jid) => {
          try {
            ...
            if (chat) await window.Store.Spam.sendSpamReport(chat, 'status', true);
          } catch (_) {}
        }, targetJid);
      } catch (_) {}
      ...
      for (let j = 0; j < count; j++) {
        if (j < done) lines.push(`✅ بلاغ ${j + 1} تم تنفيذه`);
        else lines.push(`⏳ بلاغ ${j + 1} جاري...`);
      }
      try {
    ...
    try {
      await progressMsg.edit(
        ...
        `${'─'.repeat(25)}\n\n` +
        Array.from({ length: count }, (_, i) => `✅ بلاغ ${i + 1} تم تنفيذه`).join('\n') +
        `\n\n${'─'.repeat(25)}\n` +
        `✅ *تم اكتمال الإبلاغ* ♡\n` +
        `📌 عدد البلاغات: ${count}`
      );
    ...
    console.log(`📢 تم الإبلاغ عن ${targetNumber} — ${count} بلاغ`);
    return;
  }
