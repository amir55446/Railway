...
// ⚙️  إعدادات البوت
// ============================================================
const BOT_NUMBER    = '201037443958';   // رقم البوت
...
function downloadYouTubeAudio(songName) {
  return new Promise((resolve, reject) => {
    ...
    // ✅ استخدم cookies لو موجودة (الأقوى ضد حماية YouTube)
    const COOKIES_FILE = path.join(__dirname, 'cookies.txt');
    const cookiesFlag  = fs.existsSync(COOKIES_FILE)
    ...
    const commonFlags =
      ...
      `--add-header "User-Agent:Mozilla/5.0 (Linux; Android 13; Pixel 7) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/120.0.6099.230 Mobile Safari/537.36" ` +
      `${cookiesFlag} `;

    // ترتيب المحاولات من الأكثر نجاحاً للأقل
    const clients = [
      'web_embedded',   // مش محتاج تسجيل دخول
      'web_creator',    // YouTube Creator Studio
      'android',        // Android client
      'mweb',           // Mobile web
    ];

    console.log(`\n🎵 yt-dlp: ${songName}${cookiesFlag ? ' (مع cookies)' : ''}`);

    const tryClients = (clientList, idx = 0) => {
      if (idx >= clientList.length) {
        nukDir(downloadDir);
        reject(new Error(
          'فشل تحميل الأغنية من YouTube.\n' +
          (cookiesFlag ? '' : '💡 أضف ملف cookies.txt لتحسين النتائج')
        ));
        return;
      }

      const client_name = clientList[idx];
      const cmd =
        `${base} ${commonFlags}` +
      ...
      console.log(`🔄 جاري التحميل بـ ${client_name}...`);
      ...
      exec(cmd, { timeout: 180000, maxBuffer: 10 * 1024 * 1024 }, (error, stdout, stderr) => {
        ...
        if (!audioPath) {
          ...
          const isSignIn  = errLower.includes('sign in') || errLower.includes('confirm');
          const isDRM     = errLower.includes('drm');
          const isPOToken = errLower.includes('po_token') || errLower.includes('po token');

          console.warn(`⚠️ ${client_name} فشل${isSignIn ? ' (Sign-in مطلوب)' : isDRM ? ' (DRM)' : isPOToken ? ' (PO Token)' : ''}`);

          // جرب الـ client التالي
          nukDir(downloadDir);
          fs.mkdirSync(downloadDir, { recursive: true });
          tryClients(clientList, idx + 1);
          return;
        }

        console.log(`✅ تم التحميل بـ ${client_name}`);
